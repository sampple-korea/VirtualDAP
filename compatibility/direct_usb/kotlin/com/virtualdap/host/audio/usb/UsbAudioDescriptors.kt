...
/** UAC2 clock entities are kept as a graph because a terminal may point at a selector or multiplier. */
sealed class UsbAudioClockEntity(open val id: Int, open val controls: Int) {
    data class Source(
        override val id: Int,
        val attributes: Int,
        override val controls: Int,
    ) : UsbAudioClockEntity(id, controls)
...
data class UsbAudioClockTopology(
    val rootEntity: Int,
    val entities: Map<Int, UsbAudioClockEntity>,
) {
    init {
        require(rootEntity in 1..255) { "Invalid UAC2 terminal clock entity" }
        require(entities.size <= 255 && entities.all { (id, entity) -> id == entity.id && id in 1..255 }) {
            "Invalid UAC2 clock topology"
        }
    }
}
...
data class UsbAudioStreamingProfile(
    ...
    val clockTopology: UsbAudioClockTopology? = null,
) {
...
object UsbAudioDescriptors {
    fun parse(raw: ByteArray): List<UsbAudioStreamingProfile> {
        ...
        val terminalClocks = mutableMapOf<Triple<Int, Int, Int>, Int>()
        val clockEntities = mutableMapOf<Pair<Int, Int>, MutableMap<Int, UsbAudioClockEntity>>()
        var configuration = 0
                ...
                0x24 -> {
                    ...
                    if (subclass == 1 && protocol == 0x20) {
                        if (subtype == 2 && length >= 17) {
                            terminalClocks[Triple(configuration, controlInterface, descriptor.u8(3))] = descriptor.u8(7)
                        } else if (subtype == 3 && length >= 12) {
                            terminalClocks[Triple(configuration, controlInterface, descriptor.u8(3))] = descriptor.u8(8)
                        } else if (subtype == 0x0a) {
                            require(length >= 8) { "Truncated UAC2 clock source descriptor" }
                            putClockEntity(
                                clockEntities, configuration, controlInterface,
                                UsbAudioClockEntity.Source(descriptor.u8(3), descriptor.u8(4), descriptor.u8(5)),
                            )
                        } else if (subtype == 0x0b) {
                            require(length >= 7) { "Truncated UAC2 clock selector descriptor" }
                            val pins = descriptor.u8(4)
                            require(pins in 1..248 && length >= 7 + pins) {
                                "Invalid UAC2 clock selector descriptor"
                            }
                            putClockEntity(
                                clockEntities, configuration, controlInterface,
                                UsbAudioClockEntity.Selector(
                                    descriptor.u8(3),
                                    List(pins) { pin -> descriptor.u8(5 + pin) },
                                    descriptor.u8(5 + pins),
                                ),
                            )
                        } else if (subtype == 0x0c) {
                            require(length >= 7) { "Truncated UAC2 clock multiplier descriptor" }
                            putClockEntity(
                                clockEntities, configuration, controlInterface,
                                UsbAudioClockEntity.Multiplier(
                                    descriptor.u8(3), descriptor.u8(4), descriptor.u8(5),
                                ),
                            )
                        }
                    }
        ...
        return profiles.mapNotNull { profile ->
            val clock = terminalClocks[Triple(profile.configuration, profile.controlInterface, profile.terminal)]
            val entities = clockEntities[profile.configuration to profile.controlInterface].orEmpty().toMap()
            profile.build(clock, entities)
        }
    }

    private fun putClockEntity(
        topologies: MutableMap<Pair<Int, Int>, MutableMap<Int, UsbAudioClockEntity>>,
        configuration: Int,
        controlInterface: Int,
        entity: UsbAudioClockEntity,
    ) {
        require(controlInterface in 0..255 && entity.id in 1..255) { "Invalid UAC2 clock entity" }
        val entities = topologies.getOrPut(configuration to controlInterface, ::mutableMapOf)
        val previous = entities.putIfAbsent(entity.id, entity)
        require(previous == null || previous == entity) { "Duplicate UAC2 clock entity ${entity.id}" }
    }
    ...
    private class Builder(
        ...
        fun build(clock: Int?, clockEntities: Map<Int, UsbAudioClockEntity>): UsbAudioStreamingProfile? {
            ...
            return UsbAudioStreamingProfile(
                ...
                clockTopology = clock?.takeIf { protocol == 0x20 }?.let {
                    UsbAudioClockTopology(it, clockEntities)
                },
            )
        }
    }
